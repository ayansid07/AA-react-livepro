...
exports.activatePlan = async (req, res) => {
    try {
        ...
        if (!planStatus) {
            return res.status(400).json({ message: 'planStatus are required' });
        }
        const subscription = await subscriptionModel.findById(_id);
        if (!subscription) {
            return res.status(404).json({ message: 'Subscription not found for userId ' + _id });
        }
        subscription.planStatus = planStatus;

        await subscription.save();

        return res.status(200).json({ message: 'Plan is activated successfully' });
    } catch (error) {
        return res.status(500).json({ message: error.message });
    }
}
